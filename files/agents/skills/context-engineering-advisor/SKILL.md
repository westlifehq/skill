...
description: Diagnose context stuffing vs context engineering. Use when improving AI agent context, memory architecture, retrieval boundaries, or Research-Plan-Reset-Implement workflows.
argument-hint: "Describe the workflow, agent, or context problem to diagnose"
---
...
Use a lightweight guided protocol:
- Start by asking whether the user wants Guided diagnosis, Context dump, or Best-guess assessment.
- Ask one question at a time when information is missing.
- Use numbered options for decision points.
- Keep the final diagnosis actionable: symptoms, root cause, better context boundary, retrieval/memory strategy, and next changes.
