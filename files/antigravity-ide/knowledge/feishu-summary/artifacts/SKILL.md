...
3. **创建飞书文档**：
   ...
   - **执行命令**：
     ```bash
     # ⚠️ 必须确保环境变量加载（针对 NVM 环境）
     export PATH="/Users/hq/.nvm/versions/node/v24.14.1/bin:\$PATH"
     lark-cli docs +create --title "[你的标题]" --markdown "[生成的Markdown内容]" --folder-token "PLmPfbXk7ltad0dOy9pcHIYEn7I"
     ```
   - **⚠️ 环境性要求 (ENVIRONMENT)**：执行前必须核实 `node` 与 `lark-cli` 是否在 PATH 中。若缺失，必须手动将 NVM 的 bin 目录（默认：`/Users/hq/.nvm/versions/node/v24.14.1/bin`）注入。
   - **⚠️ 最高优先级指令 (CRITICAL)**：调用 `lark-cli docs +create` 时，**必须且永远**携带 `--folder-token "PLmPfbXk7ltad0dOy9pcHIYEn7I"` 参数，确保文件存入 `ai` 外部共享文件夹，禁止漏传导致存入根目录。
